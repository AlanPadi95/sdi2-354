...
public class BeanAdministrador implements Serializable {
	...
	public String changeStatus(User user) {
		try {
			if (user.getStatus().equals(UserStatus.DISABLED)) {
				Services.getAdminService().enableUser(user.getId());
				System.out.println("DEBUG: desbloqueando el usuario "
						+ user.getLogin());
				listUsers();
				return "exito";
			} else {
				Services.getAdminService().disableUser(user.getId());
	...
	public String listUsers() {
		try {
			...
			List<User> aux = Services.getAdminService().findAllUsers();
			List<User> aux2 = Services.getAdminService().findAllUsers();
	...
	public String bajaUsuario() {
		try {
			...
			Services.getAdminService().deepDeleteUser(selectedUser.getId());
