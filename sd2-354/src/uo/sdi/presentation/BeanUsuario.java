...
public class BeanUsuario {
	...
	public String addTarea() {
		...
		try {
			...
			try {
				Services.getTaskService().createTask(task);
				System.out.println("DEBUG: El usuario "
						+ obtenerUsuario().getLogin() + " ha añadido la tarea "
						+ task.getTitle() + " con categoría" + category);
				vaciarCampos();
	...
	public List<String> cargarCategorias() {
		...
		try {
			this.cS = Services.getTaskService().findCategoriesByUserId(
	...
	public void markAsFinished(Task task) {
		try {
			Services.getTaskService().markTaskAsFinished(task.getId());
	...
	public String listInitialTask() {
		try {
			...
			tareas = Services.getTaskService().findCreatedNotFinished(
					getUserLogued().getId());
	...
	public String listInbox() {
		try {
			...
			setTareas(Services.getTaskService().findInboxTasksByUserId(
	...
	public String listInboxNotFinished() {
		try {
			...
			setTareas(Services
					.getTaskService()
					.findInboxNotFinishedTasksByUserId(obtenerUsuario().getId()));
			setIsInbox(true);
			return "listar";
		} catch (BusinessException e) {
			System.err
	...
	public String listInboxFinished() {
		try {
			...
			setTareas(Services.getTaskService().findFinishedInboxTasksByUserId(
					obtenerUsuario().getId()));
			setIsInbox(true);
			return "listar";
	...
	public String listHoy() {
		try {
			...
			setTareas(Services.getTaskService().findTodayTasksByUserId(
					obtenerUsuario().getId()));
			setIsHoy(true);
			return "listar";
	...
	public String listSemana() {
		try {
			...
			setTareas(Services.getTaskService().findWeekTasksByUserId(
					obtenerUsuario().getId()));
			setIsSemana(true);
			return "listar";
